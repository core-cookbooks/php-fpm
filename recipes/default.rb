...
when 'amazon', 'fedora', 'centos', 'redhat'
  unless platform?('centos', 'redhat') && node['platform_version'].to_f >= 6.2
    unless platform?('fedora') && node['platform_version'].to_f >=15.0
...
if node["platform"] == "fedora" && node["platform_version"].to_f == 18.0
  bash 'installing openssl_package' do
    user "root"
    code <<-EOH
...
if platform?('ubuntu') && node[:platform_version] == '14.04'
   bash  "Enable module php5-mcrypt" do
     user "root"
     code <<-EOH
